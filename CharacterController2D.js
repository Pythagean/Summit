...
public class CharacterController2D extends MonoBehaviour
{
	...
	public class CharacterCollisionState2D
	{
		...
		var above : boolean ;
		var below : boolean ;
		var becameGroundedThisFrame : boolean ;
		var wasGroundedLastFrame : boolean ;
		var movingDownSlope : boolean ;
		var slopeAngle : float ;
		
		
		function boolean hasCollision()
		{
	...
	/// </summary>
	var usePhysicsForMovement : boolean  = false;
	
	[SerializeField]
	[Range( 0.001f, 0.3f )]
	var _skinWidth : float  = 0.02f;
	...
	/// </summary>
	var platformMask : LayerMask  = 0;
	var platformTeamMask : LayerMask  = 0;
	...
	/// </summary>
	[SerializeField]
	private var oneWayPlatformMask : LayerMask  = 0;
	...
	[Range( 0, 90f )]
	var slopeLimit : float  = 30f;
	
	
	/// <summary>
	/// curve for multiplying speed based on slope (negative = down slope and positive = up slope)
	/// </summary>
	var slopeSpeedMultiplier : AnimationCurve  = AnimationCurve( new Keyframe( -90, 1.5f ), new Keyframe( 0, 1 ), new Keyframe( 90, 0 ) );
	
	[Range( 2, 20 )]
	var totalHorizontalRays : int  = 8;
	[Range( 2, 20 )]
	var totalVerticalRays : int  = 4;
	...
	/// </summary>
	private var _slopeLimitTangent : float  = Mathf.Tan( 75f * Mathf.Deg2Rad );
	...
	/// </summary>
	var createTriggerHelperGameObject : boolean  = false;
	
	[Range( 0.8f, 0.999f )]
	var triggerHelperBoxColliderScale : float  = 0.95f;
	...
	[HideInInspector][NonSerialized]
	var collisionState : CharacterCollisionState2D  = new CharacterCollisionState2D();
	[HideInInspector][NonSerialized]
	var velocity : Vector3 ;
	var isGrounded : boolean  { get { return collisionState.below; } }
	...
	/// </summary>
	private var _raycastOrigins : CharacterRaycastOrigins ;
	
	/// <summary>
	/// stores our raycast hit during movement
	/// </summary>
	private var _raycastHit : RaycastHit2D ;
	private var _raycastHitTeam : RaycastHit2D ;
	...
	private var _verticalDistanceBetweenRays : float ;
	private var _horizontalDistanceBetweenRays : float ;
	...
	private var _isGoingUpSlope : boolean  = false;
	...
	public function OnTriggerStay2D( col : Collider2D  )
	{
	...
	public function OnTriggerExit2D( col : Collider2D  )
	{
	...
	public function move( deltaMovement: Vector3  )
	{
	...
	/// </summary>
	public function warpToGrounded()
	{
		do
		{
	...
	/// </summary>
	public function recalculateDistanceBetweenRays()
	{
	...
	/// <param name="deltaMovement">Delta movement.</param>
	private function primeRaycastOrigins( futurePosition : Vector3 , deltaMovement : Vector3  )
	{
		var scaledColliderSize = Vector2( boxCollider.size.x * Mathf.Abs( transform.localScale.x ), boxCollider.size.y * Mathf.Abs( transform.localScale.y ) ) / 2;
		var scaledCenter = Vector2( boxCollider.offset.x * transform.localScale.x, boxCollider.offset.y * transform.localScale.y );
	...
	private function moveHorizontally( deltaMovement : Vector3  )
	{
		...
		{
			var ray = Vector2( initialRayOrigin.x, initialRayOrigin.y + i * _verticalDistanceBetweenRays );
